...
public class UserLocation extends Service {
	...
	private static float savedLongitude = (float) 0.000000;
	private static float savedLatitude = (float) 0.000000;
	private static long savedTimeStamp = 0;
	private static float savedAccuracy = (float) 0.0;
	private static String savedProvider = null;
	...
	private void initSettings() {
		// Restore preferences
		settings = getSharedPreferences(SAVED_LOCATION_NAME, 0);
		savedLongitude = settings.getFloat(LONGITUDE_KEY, savedLongitude);
		savedLatitude = settings.getFloat(LATITUDE_KEY, savedLatitude);
		savedAccuracy = settings.getFloat(ACCURACY_KEY, savedAccuracy);
		savedProvider = settings.getString(PROVIDER_KEY, savedProvider);
		savedTimeStamp = settings.getLong(TIME_STAMP_KEY, savedTimeStamp);
	...
	private final class ServiceHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			try {
				...
				// public void run() {
				// getCurrentLocation();
				// //UserLocation.this.getCurrentLocation();
				// }
				// }, (long) 1000, tempInt);
